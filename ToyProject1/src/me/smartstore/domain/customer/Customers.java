...
public class Customers {
	...
	public Customer[] getRealCustomers() {
		...
		this.size = real;
		return Arrays.<Customer>copyOf(this.customers, real);
	}
	...
	public int indexOf(Customer customer) {
		...
		for (int i = 0; i < this.size; i++) {
			if (this.customers[i] != null && 
					this.customers[i].equals(customer))
				return i;
		} 
		return -1;
	}
	...
	public void grow() {
		Customer[] copy = Arrays.<Customer>copyOf(this.customers, this.customers.length);
		this.capacity *= 2;
	...
	public Customers trimToSize() {
		Customer[] newCustomers = new Customer[this.size];
		System.arraycopy(this.customers, 0, newCustomers, 0, this.size);
	...
	public void print() {
		for (int i = 0; i < this.size; i++) {
			if (this.customers[i] != null)
				System.out.printf("No.  %4d => %s\n", new Object[] { Integer.valueOf(i + 1), this.customers[i] }); 
		}
	}
	...
	public ClassifiedCustomersGroup classified() {
		...
		for (int i = 0; i < this.allGroups.size(); i++) {
			...
			Customer[] copy = Arrays.<Customer>copyOf(customers, customers.length);
