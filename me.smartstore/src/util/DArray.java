...
import handler.exception.ElementNotFoundException;
import handler.exception.EmptyArrayException;
import handler.exception.NullArgumentException;

public class DArray<T> implements Collections<T> { // Dynamic Array

    //static 필드는 가장 위로 올려주는 게 좋다.
    ...
    public int indexOf(T object) throws NullArgumentException, ElementNotFoundException {
        if (object == null) throw new NullArgumentException(); // not found (instead of throwing handler.exception)
